    ...
    def _parseAdvEntry(self, advinfo):
        ...
        for cid in advinfo['cves']:
            ...
            if cid.startswith('CVE'):
                _adv['cves'][cid] = dict()
                _adv['cves'][cid]['url'] = self.cveurl.format(cid.split('-')[1],cid)
            if self.cvebase.has_key(cid):
                _adv['cves'][cid]['subject'] = self.cvebase[cid]['subject']
                _adv['cves'][cid]['references'] = self.cvebase[cid]['references']
                _adv['cves'][cid]['commits'] = [l for l in self.cvebase[cid]['references'] if 'commit' in l]
        _adv['packages'] = list()
        for dist in self.dists:
            if advinfo['releases'].has_key(dist):
                sources = advinfo['releases'][dist]['sources']
                for pkg,pkginfo in sources.iteritems():
                    if pkg in self.pkgbase[self.project_map[dist]]:
                        epoch, upstream_version, revision, full_version = self._parseVersion(pkginfo['version'])
...
class RedhatAdvisories(Advisories):
    ...
    def addAdvisoriesInfo(self):
        for adventry in self.advisories:
            if len([x for x in adventry['released_packages'] for y in self.dists if x.find(y) > 0]) > 0:
    ...
    def _parseAdvEntry(self,adventry):
        ...
        for dist in self.dists:
            for pkg in adventry['released_packages']:
                if dist in pkg:
                    name, epoch, upstream_version, revision, full_version = self._parseVersion(pkg)
    ...
    def _parseVersion(self,pkg):
        re_valid_version = re.compile("(\S+)-(?:(\d*):)?(.*)-(~?\w+[\w.]*)?(\.(el.*))")
        try:
            name, epoch, upstream_version, revision, _, release = re_valid_version.match(pkg).groups()
            if isinstance(epoch, type(None)):
                full_version = '{0}-{1}'.format(upstream_version, revision)
                epoch = '0'
            else:
                full_version = '{0}:{1}-{2}'.format(epoch,upstream_version,revision)
